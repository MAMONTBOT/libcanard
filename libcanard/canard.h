...
#define CANARD_TRANSFER_ID_COUNT      (1U << CANARD_TRANSFER_ID_BIT_LENGTH)
#define CANARD_TRANSFER_ID_MAX        (CANARD_TRANSFER_ID_COUNT - 1U)
...
/// There shall be a most one publisher instance per topic at any given time, and they shall not be moved/copied.
/// Instances should not be created ad-hoc to publish a message because they retain the transfer-ID state between
/// publications.
typedef struct canard_publisher_t
{
    canard_tree_t index_topic_hash; ///< Must be the first member.

    uint64_t topic_hash;

    uint_least8_t kind;
    uint_least8_t next_transfer_id;

    uint32_t        transfer_id_occupancy_bitmap;
    canard_txfer_t* transfers_by_id[CANARD_TRANSFER_ID_COUNT];
...
typedef struct canard_vtable_t
{
    ...
    uint32_t (*tx_subject_id)(canard_t*, canard_publisher_t*);
...
struct canard_t
{
    ...
    struct
    {
        ...
        canard_tree_t* publishers_by_topic;

        canard_tree_t* transfers_pending_by_can_id[CANARD_IFACE_COUNT]; ///< Next to transmit on the left.
        canard_tree_t* transfers_staged_by_until;                       ///< Earliest to retransmit on the left.
        canard_list_t  transfers_agewise;                               ///< ALL transfers, oldest at the head.

        canard_publisher_t* iterator; ///< For iterative poll() scanning; NULL to restart.
    } tx;
...
/// Destroy publishers and subscribers first.
